...
class add_driver extends uvm_driver #(add_seq_item);
  ...
  task run_phase(uvm_phase phase);
    ...
    initialize();
    forever begin
      `uvm_info("DEBUG: add_driver Run", "... getting next item ...", UVM_FULL)
      seq_item_port.get_next_item(tr);
      drive_item(tr);
      seq_item_port.item_done();
      `uvm_info("DEBUG: add_driver Run", "... next item done ...", UVM_FULL)
    end
  endtask

  virtual task initialize (); // @0 - Does not use clocking block
    `uvm_info("INIT", "add_driver: Initialize (time @0)", UVM_HIGH)
    vif.a <= '1;
    vif.b <= '1;
    @vif.cb1;
  ...
  virtual task drive_item (add_seq_item tr);
