...
class wr_driver extends uvm_driver #(wr_item);
  ...
  virtual task drive_item (wr_item tr);
    ...
    m_responses++;
    vif.cb1.bvalid <= tr.bvalid;
    vif.cb1.bid <= tr.bid;
    vif.cb1.bresp <= tr.bresp;
    @vif.cb1;
    // pragma quickuvm custom drive_item_additional begin
    // The B channel is valid for exactly ONE cycle per response: drop bvalid the cycle after
    // we asserted it, so each B beat is a distinct pulse the DUT master can count.
    vif.cb1.bvalid <= 1'b0;
    // Model a write-commit latency: a real slave commits the write before answering, so B
    // beats are SPACED. While this response is committing, the responder's accept thread keeps
    // buffering the writes that complete — so a backlog builds and the lowest-id-first
    // scheduler drains it OUT OF ORDER (the whole point of `respond: pipelined`).
    repeat (4) @vif.cb1;
