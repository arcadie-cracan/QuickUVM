...
class rd_driver extends uvm_driver #(rd_item);
  ...
  virtual task drive_item (rd_item tr);
    ...
    // One-cycle valid pulse, then a modeled read latency so a backlog of outstanding
    // requests accumulates and the reorder_policy has several ids to choose between.
    vif.cb1.rvalid <= 1'b0;
    @vif.cb1;
    repeat (8) @vif.cb1;
