...
class sndr_driver extends uvm_driver #(sndr_item);
  ...
  virtual task drive_item (sndr_item tr);
    ...
    // De-assert the strobes after each item so `resp`/`alert` PULSE for one cycle instead
    // of holding (the on_request driver parks the pins at the last item between items). A
    // held `alert` would make the DUT latch every cycle; a held `resp` would over-count.
    vif.cb1.resp  <= 1'b0;
    vif.cb1.alert <= 1'b0;
    @vif.cb1;
