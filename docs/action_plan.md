...
Status: **Track A (pragma-preservation hardening) — DONE.** Track B (functional gaps) —
partially shipped (RAL complete); remainder superseded by the general-DV plan in
[`parity_roadmap.md`](parity_roadmap.md). This file is retained as the Track-A record and
the original B-phase scoping.
...
- **Track A complete; suite green** (98 tests as of v0.9.1, growing per phase).
- **Track B partially shipped since:** the register model landed in full
  (front-door v0.5.0, backdoor v0.6.0, custom front-door v0.8.0) plus a UVM-version
  selector (v0.7.0), config objects (F1, v0.3.0), and the MVP analysis fabric
  (C1, v0.4.0). See the per-version table and the reprioritized general-DV plan in
...
# Track B — Close functional gaps (original scoping; superseded by parity_roadmap.md)

> The B-phases below were scoped from the SPI-bridge bring-up. B1 shipped in full; the
> rest are re-scoped and re-prioritized for general-purpose DV in
> [`parity_roadmap.md`](parity_roadmap.md) (which leads with stimulus + coverage, not RAL).

### Phase B1 — Register model — **DONE** (C4a/b/c)
- Consumes external reggen output (the `uvm_reg_block` is not generated by QuickUVM).
- Generates: bus→reg adapter skeleton, predictor wiring, front-door / backdoor /
  custom-front-door, and an optional hw_reset/bit-bash `reg_test`.
- `register_model:` config block in `models.py`.
- Shipped v0.5.0 (front-door) → v0.6.0 (backdoor) → v0.8.0 (custom front-door),
  with the UVM-version selector in v0.7.0.

### Phase B2 — Topology beyond single flat agent — **PARTIAL** (C1 MVP)
- Opt-in `analysis:` per-agent routing shipped (v0.4.0), fixing the hardwired-`agents[0]`
  assumption for routed agents. Full sub-environments/hierarchy (H1) and the
  multi-predictor fabric remain — see roadmap tiers 2–3.

### Phase B3 — Parameterization & richer tests — **PENDING**
- Re-scoped into roadmap S1 (rich transactions/constraints), S2 (sequence library),
  C2 (virtual sequences), C3 (parameterization).

### Phase B4 — Run/regression infrastructure — **PENDING**
- Re-scoped into roadmap R1 (per-sim makefiles, regression runner, coverage-merge).
