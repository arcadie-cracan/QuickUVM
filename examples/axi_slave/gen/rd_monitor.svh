...
class rd_monitor extends uvm_monitor;
  ...
  // UVM ORACLE. The automated verdict is UVM-severity based, so the DUT's $error checks are
  // invisible to it — make the read channel's correctness verdict-carrying: every R beat has a
  // matching outstanding id, all are delivered, and a cross-id REORDER actually occurred (the
  // out-of-order property this channel exists to show; deterministic for this DUT + latency).
  bit       m_r_out [16];
  bit [3:0] m_r_issue [$];
  bit [3:0] m_r_recv  [$];
  int       m_r_got;
