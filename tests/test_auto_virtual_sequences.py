...
def test_auto_triggers_for_two_active_agents():
    ...
    assert cfg.auto_vseq_name == "sub_vseq"
    vseqs = cfg.effective_virtual_sequences
    assert len(vseqs) == 1
    assert vseqs[0].name == "sub_vseq"
    assert vseqs[0].mode == "parallel"
    assert [s.agent for s in vseqs[0].body] == ["a", "b"]
    assert [s.sequence for s in vseqs[0].body] == ["a_sequence", "b_sequence"]
...
def test_explicit_vseqs_win():
    explicit = VseqConfig(
        name="my_vseq", body=[VseqStep(agent="a", sequence="a_sequence")]
    )
...
def test_auto_generates_vsqr_and_default_vseq(tmp_path):
    ...
    assert (tmp_path / "env_vsqr.svh").exists()
    assert (tmp_path / "env_vseq_base.svh").exists()
    vs = (tmp_path / "sub_vseq.svh").read_text()
    assert "class sub_vseq extends env_vseq_base;" in vs
...
def test_invalid_project_name_rejected():
    # project.name derives the auto vseq class name, so it must be an identifier
    with pytest.raises(Exception, match="legal SystemVerilog identifier"):
        ProjectMeta(name="my-proj")


def test_test_may_reference_auto_vseq_by_name(tmp_path):
    # a test can name the auto-default vseq explicitly (project 'sub' -> sub_vseq)
    cfg = _cfg([_agent("a"), _agent("b")], tests=[TConf(name="t", vseq="sub_vseq")])
    Generator(cfg).generate_all(tmp_path)
