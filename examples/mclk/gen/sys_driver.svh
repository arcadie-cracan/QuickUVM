...
class sys_driver extends uvm_driver #(sys_item);
  ...
  task run_phase(uvm_phase phase);
    ...
    initialize();
    forever begin
      `uvm_info("DEBUG: sys_driver Run", "... getting next item ...", UVM_FULL)
      seq_item_port.get_next_item(tr);
      drive_item(tr);
      seq_item_port.item_done();
      `uvm_info("DEBUG: sys_driver Run", "... next item done ...", UVM_FULL)
    end
  endtask

  virtual task initialize (); // @0 - Does not use clocking block
    `uvm_info("INIT", "sys_driver: Initialize (time @0)", UVM_HIGH)
    vif.sys_din <= '1;
  ...
  virtual task drive_item (sys_item tr);
    `uvm_info("drive_item", tr.input2string(), UVM_FULL)
    vif.cb1.sys_din <= tr.sys_din;
    @vif.cb1;
