...
class cha_driver extends uvm_driver #(cha_seq_item);
  `uvm_component_utils(cha_driver)
  ...
  task run_phase(uvm_phase phase);
    ...
    initialize();
    forever begin
      `uvm_info("DEBUG: cha_driver Run", "... getting next item ...", UVM_FULL)
      seq_item_port.get_next_item(tr);
      drive_item(tr);
      seq_item_port.item_done();
      `uvm_info("DEBUG: cha_driver Run", "... next item done ...", UVM_FULL)
    end
  endtask

  virtual task initialize (); // @0 - Does not use clocking block
    `uvm_info("INIT", "cha_driver: Initialize (time @0)", UVM_HIGH)
  ...
  virtual task drive_item (cha_seq_item tr);
