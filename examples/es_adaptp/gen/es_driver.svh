...
class es_driver extends uvm_driver #(es_item);
  `uvm_component_utils(es_driver)
  ...
  task run_phase(uvm_phase phase);
    ...
    initialize();
    forever begin
      `uvm_info("DEBUG: es_driver Run", "... getting next item ...", UVM_FULL)
      seq_item_port.get_next_item(tr);
      drive_item(tr);
      seq_item_port.item_done();
      `uvm_info("DEBUG: es_driver Run", "... next item done ...", UVM_FULL)
    end
  endtask

  virtual task initialize (); // @0 - Does not use clocking block
    `uvm_info("INIT", "es_driver: Initialize (time @0)", UVM_HIGH)
    vif.sample <= '1;
    vif.valid <= '0;
  ...
  virtual task drive_item (es_item tr);
    `uvm_info("drive_item", tr.input2string(), UVM_FULL)
    vif.cb1.sample <= tr.sample;
    vif.cb1.valid <= tr.valid;
    @vif.cb1;
