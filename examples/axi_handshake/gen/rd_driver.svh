...
class rd_driver extends uvm_driver #(rd_item);
  ...
  virtual task initialize (); // @0 - Does not use clocking block
    ...
    vif.arready <= 1'b1;   // this slave is always ready to accept a read-address request
  ...
  virtual task drive_item (rd_item tr);
    ...
    vif.cb1.arready <= tr.arready;
    @vif.cb1;
    // pragma quickuvm custom drive_item_additional begin
    // R-channel HANDSHAKE: valid is HELD until ready. Keep rvalid asserted until the master
    // accepts the beat (rready high), then drop it — this is the response-side backpressure.
    while (!vif.cb1.rready) @vif.cb1;
    vif.cb1.rvalid <= 1'b0;
