...
class rd_driver extends uvm_driver #(rd_item);
  ...
  virtual task drive_item (rd_item tr);
    ...
    // The R channel is valid for exactly ONE cycle per beat: drop rvalid the cycle after we
    // drove it, so back-to-back responses are distinct pulses the master captures once each.
    vif.cb1.rvalid <= 1'b0;
    @vif.cb1;
    // Modeled slave READ LATENCY. A real slave does not answer instantly; this delay lets a
    // backlog of outstanding requests accumulate, so the responder's lowest-id-first policy
    // has something to reorder (without it, each request is answered before the next lands).
    repeat (8) @vif.cb1;
